...
typedef struct flash_options {
	...
	FlashReg_pt	base_address_control;
...
typedef struct flash_device {
	device_t device; 				/**< The generic device. */
	FlashReg_pt membase; 			/**< The memory base for the device's control registers. */
	IRQn_Type irq; 					/**< The IRQ number of the IRQ associated to the device. */
	uint32_t array_base_address; 	/**< Base address of the array */
} flash_device_t, *flash_device_pt;
